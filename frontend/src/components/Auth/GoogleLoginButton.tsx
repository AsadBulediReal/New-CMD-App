...
export const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn("Error rendering native Google button:", e);
    }
  }, [scriptLoaded, clientId, text, handleCredentialResponse]);

  const handleCustomButtonClick = () => {
    if (!clientId) {
      toast.info("Google OAuth is being set up. Please set VITE_GOOGLE_CLIENT_ID in your environment variables.", {
        duration: 5000,
      });
      return;
    }
  ...
  return (
    <div className={`w-full ${className}`}>
      {/* If Client ID is configured and GIS loaded, render the official Google button container */}
      {clientId && scriptLoaded ? (
        ...
        /* Custom branded fallback button */
        <button
          type="button"
          onClick={handleCustomButtonClick}
          disabled={disabled || loading}
          className="w-full min-h-[44px] py-2.5 px-4 bg-background hover:bg-muted/70 active:scale-[0.99] border border-border/80 text-foreground font-medium rounded-xl text-sm transition-all flex items-center justify-center gap-3 shadow-xs cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? (
